...
export default function Page() {
  ...
  const [years, setYears] = useState<number[]>([2018, 2019, 2020]);

  const [singleChoiceCount, setSingleChoiceCount] = useState(40);
  const [judgmentCount, setJudgmentCount] = useState(2);
  ...
  return (
    <>
      <PageHeading
        title="生成随机试卷"
        description="从2018—2020年真题题库中随机抽题，生成一套练习试卷。"
      />
              ...
              {YEARS.map((year) => {
                ...
                return (
                  <button
                    ...
                    className={
                      selected
                        ? "rounded-xl border border-slate-900 bg-slate-900 px-5 py-2.5 text-sm font-medium text-white"
                        : "rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50"
                    }
                  >
                    {year} 年
                  </button>
          ...
          <section className="card p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  可以使用标准成人高考结构，也可以自行调整题量。
                </p>
              </div>

              <button
                type="button"
                onClick={resetStandardExam}
